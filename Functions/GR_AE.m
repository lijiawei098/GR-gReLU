function Parameters = GR_AE(Magnitude)
%GR_AE Estimate augmented GR_AE model parameters with bootstrap uncertainty.
%   Parameters = GR_AE(Magnitude)
%   Input:
%       Magnitude : N-by-1 (or 1-by-N) vector of earthquake magnitudes.
%   Output (struct):
%       .names        : {'a','b','mc','sigma_mc','theta'}
...
%   Version 2.1: May 1, 2026.
    ...
    if nargin < 1
        error('GR_AE:MissingInput', 'Magnitude input is required.');
    end
    ...
    if isempty(positive_y)
        error('GR_AE:InvalidCCFMD', 'CCFMD is all zeros; cannot fit model.');
    end
    ...
    lb = [a_lb, b_lb, mc_lb, sigma_lb, theta_lb];
    ub = [a_ub, b_ub, mc_ub, sigma_ub, theta_ub];

    a0 = log10(max(y(1), 1));
    b0 = 1.0;
    sigma0 = max(0.2, min(0.6, 0.25 * m_rng));
    theta0 = 1.0;
    p0 = [a0, b0, mc0, sigma0, theta0];
    p0 = max(lb, min(ub, p0));

    model = @(p, mm) local_GRLUE_AE(mm, p(1), p(2), p(3), p(4), p(5));
    ...
    if isempty(boot_params)
        error('GR_AE:BootstrapFailed', 'All bootstrap inversions failed.');
    end

    Parameters = struct();
    Parameters.names = {'a', 'b', 'mc', 'sigma_mc', 'theta'};
...
function GR_values = local_GRLUE_AE(m, a, b, m_c, sigma_m, theta)
    activation_value = local_GRAE(m, m_c, sigma_m, theta);
    GR_values = a - b * m_c - b * sigma_m * activation_value;
end

function y = local_GRAE(m, m_c, sigma_m, theta)
    x = (m - m_c) ./ sigma_m;
    y = zeros(size(x));
    idx_neg = x <= 0;
    idx_pos = ~idx_neg;
    y(idx_neg) = exp(x(idx_neg)) ./ (1 + theta);
    y(idx_pos) = x(idx_pos) + exp(-theta .* x(idx_pos)) ./ (1 + theta);
end
