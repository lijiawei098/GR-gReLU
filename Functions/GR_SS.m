function Parameters = GR_SS(Magnitude)
%GR_SS Estimate augmented GR_SS model parameters with bootstrap uncertainty.
...
%   Version 2.1: May 1, 2026.
    ...
    if nargin < 1
        error('GR_SS:MissingInput', 'Magnitude input is required.');
    end
    ...
    if isempty(positive_y)
        error('GR_SS:InvalidCCFMD', 'CCFMD is all zeros; cannot fit model.');
    end
    ...
    model = @(p, mm) local_GRLUE_SS(mm, p(1), p(2), p(3), p(4));
    ...
    if isempty(boot_params)
        error('GR_SS:BootstrapFailed', 'All bootstrap inversions failed.');
    end
...
function GR_values = local_GRLUE_SS(m, a, b, m_c, sigma_m)
    activation_value = local_GRSS((m - m_c) ./ sigma_m);
    GR_values = a - b * m_c - b * sigma_m * activation_value;
end

function y = local_GRSS(x)
    y = log(1 + exp(x));
end
