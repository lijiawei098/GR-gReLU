function Parameters = GR_BS(Magnitude)
%GR_BS Estimate augmented GR_BS model parameters with bootstrap uncertainty.
%   Parameters = GR_BS(Magnitude)
%   Input:
%       Magnitude : N-by-1 (or 1-by-N) vector of earthquake magnitudes.
%   Output (struct):
%       .names        : {'a','b','mc','sigma_mc','C'}
%       .estimate     : point estimate from full sample [1x5]
%       .std          : bootstrap standard deviation [1x5]
%       .ci95         : percentile 95% CI [5x2]
%       .bounds       : struct with lower/upper bounds used in inversion
%       .bootstrap    : successful bootstrap parameter samples [Kx5]
...
%   Version 2.1: May 1, 2026.
    ...
    if nargin < 1
        error('GR_BS:MissingInput', 'Magnitude input is required.');
    end
    ...
    if isempty(positive_y)
        error('GR_BS:InvalidCCFMD', 'CCFMD is all zeros; cannot fit model.');
    end
    ...
    % C shifts magnitudes to keep BS term well-defined and flexible.
    c_lb = 0.0;
    c_ub = max(0.8, min(5.0, m_rng + 1.0));

    a_ref = log10(numel(Magnitude)) + b_lb * mc_lb;
    a_lb = max(-2, a_ref - 6);
    a_ub = a_ref + 8;

    lb = [a_lb, b_lb, mc_lb, sigma_lb, c_lb];
    ub = [a_ub, b_ub, mc_ub, sigma_ub, c_ub];

    a0 = log10(max(y(1), 1));
    b0 = 1.0;
    sigma0 = max(0.2, min(0.6, 0.25 * m_rng));
    c0 = 0.1;
    p0 = [a0, b0, mc0, sigma0, c0];
    p0 = max(lb, min(ub, p0));

    model = @(p, mm) local_GRLUE_BS(mm, p(1), p(2), p(3), p(4), p(5));
    ...
    if isempty(boot_params)
        error('GR_BS:BootstrapFailed', 'All bootstrap inversions failed.');
    end

    Parameters = struct();
    Parameters.names = {'a', 'b', 'mc', 'sigma_mc', 'C'};
...
function GR_values = local_GRLUE_BS(m, a, b, m_c, sigma_m, C)
    activation_value = local_GRBS(m, m_c, sigma_m, C);
    GR_values = a - b * (m_c + C) - b * sigma_m * activation_value;
end

function y = local_GRBS(m, m_c, sigma_m, C)
    m_translated = m + C;
    m_c_translated = m_c + C;

    m_translated = max(m_translated, eps);
    m_c_translated = max(m_c_translated, eps);
    ...
    y = (m_translated ./ sigma_m) .* normcdf(d1) - (m_c_translated ./ sigma_m) .* normcdf(d2);
end
